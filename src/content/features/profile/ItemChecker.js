...
async function injectItemChecker(container) {
    ...
    const filterConfig = [
        ...
        {
            id: 'rovalra-check-offsale',
            label: 'Include Offsale',
            type: 'toggle',
            initialValue: true,
        },
        {
            id: 'rovalra-check-limit',
            label: 'Limit',
            type: 'dropdown',
            options: [
                { value: '120', label: '120' },
    ...
    const performScan = async (
        ...
        try {
            ...
            const ownershipPromises = items.map(async (item) => {
                const owned = await checkItemOwnership(
                    userId,
                    item.id,
                    item.itemType,
                );
            ...
            const ownedItems = results
                .filter((r) => r.owned)
                .map((r) => r.item);
            const unownedItems = results
                .filter((r) => !r.owned)
                .map((r) => r.item);
            ...
            const renderItems = async (itemList, grid) => {
                if (itemList.length === 0) {
                    grid.innerHTML = DOMPurify.sanitize(
                        '<p style="grid-column: 1/-1; text-align: center; color: var(--rovalra-secondary-text-color);">No items found.</p>',
                    );
                    return;
                }
                ...
                itemList.forEach((item) => {
                    ...
                    const card = createItemCard(itemData, thumbMap, {
                        showOnHold: false,
                        showSerial: false,
                    });
    ...
    const onApplyFilters = () => {
        ...
        const minPrice =
            document.getElementById('rovalra-check-minprice')?.value || '';
        const limiteds =
            document
                .getElementById('rovalra-check-limiteds')
                ?.getAttribute('aria-checked') === 'true';
        const recent =
            document
                .getElementById('rovalra-check-recent')
                ?.getAttribute('aria-checked') === 'true';
        const offsale =
            document
                .getElementById('rovalra-check-offsale')
                ?.getAttribute('aria-checked') === 'true';
        const limit = parseInt(
            document.getElementById('rovalra-check-limit')?.value || '120',
        );

        if (!creatorName && !recent) {
            if (creatorInput) {
                const base = creatorInput.closest(
                    '.rovalra-catalog-input-base',
                );
                const fieldset = base?.querySelector(
...
export function init() {
    chrome.storage.local.get({ privateInventoryEnabled: false }, (settings) => {
        if (
            settings.privateInventoryEnabled &&
            window.location.href.includes('inventory')
        ) {
            observeElement(
                'div.section-content-off:not(.btr-section-content-off)',
                injectItemChecker,
            );
        }
    });
